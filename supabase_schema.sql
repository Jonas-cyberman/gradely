...
INSERT INTO grade_scale
  (grade, min_score, points, description, sort_order) VALUES
('A',  80.00, 4.00, 'EXCELLENT',    1),
('A-', 75.00, 3.75, 'VERY GOOD',    2),
('B+', 70.00, 3.50, 'GOOD',         3),
('B',  65.00, 3.00, 'CREDIT',       4),
('B-', 60.00, 2.50, 'AVERAGE',      5),
('C+', 55.00, 2.00, 'PASS',         6),
('C',  50.00, 1.50, 'PASS',         7),
('D',  40.00, 1.00, 'FAIL',         8),
('F',  1.00,  0.00, 'FAIL',         9);
...
('app_url',        'https://YOUR-NEW-VERCEL-URL.vercel.app'),
...
-- Drop old policies first (if re-running this script)
DROP POLICY IF EXISTS "Admin all programmes"      ON programmes;
DROP POLICY IF EXISTS "Admin all years"           ON years;
DROP POLICY IF EXISTS "Admin all courses"         ON courses;
DROP POLICY IF EXISTS "Admin all grade_scale"     ON grade_scale;
DROP POLICY IF EXISTS "Admin all classifications" ON classifications;
...
CREATE POLICY "Admin all programmes"
  ON programmes FOR ALL
  USING (auth.uid() IS NOT NULL)
  WITH CHECK (auth.uid() IS NOT NULL);

CREATE POLICY "Admin all years"
  ON years FOR ALL
  USING (auth.uid() IS NOT NULL)
  WITH CHECK (auth.uid() IS NOT NULL);
...
CREATE POLICY "Admin all grade_scale"
  ON grade_scale FOR ALL
  USING (auth.uid() IS NOT NULL)
  WITH CHECK (auth.uid() IS NOT NULL);

CREATE POLICY "Admin all classifications"
